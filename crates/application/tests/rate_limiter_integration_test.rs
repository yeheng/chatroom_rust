...
async fn test_rate_limiter_integration() {
    let limiter = Arc::new(MessageRateLimiter::new(5, 3)); // 5 msg/min, 3 connections
    ...
    // 第4个连接应该被拒绝
    match limiter.add_connection(user_id) {
    ...
    // 测试消息限流
    for i in 0..5 {
        match limiter.check_message_rate(user_id) {
    ...
    // 检查用户状态
    let (message_count, connection_count) = limiter.get_user_status(user_id).unwrap();
...
async fn test_rate_limiter_cleanup() {
    ...
    let (initial_count, _) = limiter.get_user_status(user_id).unwrap();
    assert_eq!(initial_count, 3);

    // 清理过期配额
    limiter.cleanup_expired_quotas();

    // 重置用户配额
    limiter.reset_user_quota(user_id);

    // 检查配额是否被重置
    let (reset_count, _) = limiter.get_user_status(user_id).unwrap_or((0, 0));
...
async fn test_concurrent_rate_limiting() {
    let limiter = Arc::new(MessageRateLimiter::new(10, 5));
    let user_id = UserId::from(Uuid::new_v4());
    ...
    for i in 0..15 {
        ...
        let handle = tokio::spawn(async move {
            let result = limiter_clone.check_message_rate(user_id);
...
async fn test_connection_lifecycle() {
    let limiter = MessageRateLimiter::new(10, 2);
    let user_id = UserId::from(Uuid::new_v4());
    ...
    let (_, connections) = limiter.get_user_status(user_id).unwrap();
    ...
    if let Some((_, connections)) = status {
