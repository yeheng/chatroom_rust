...
use application::{MessageRateLimiter, RedisPresenceManager, PresenceManager};
...
async fn test_concurrent_connection_consistency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 并发连接用户
    let connect_tasks: Vec<_> = user_ids.iter().enumerate().map(|(i, &user_id)| {
    ...
    // 等待所有连接任务完成
    let connect_results: Vec<_> = futures::future::join_all(connect_tasks).await
        .into_iter()
        .map(|r| r.unwrap())
        .collect();
    ...
    // 并发断开连接
    let disconnect_tasks: Vec<_> = user_ids.iter().enumerate().map(|(i, &user_id)| {
        let presence_manager = services.presence_manager.clone();
        let room_id = room_id;
        tokio::spawn(async move {
            // 模拟断开延迟
    ...
    // 等待所有断开任务完成
    let disconnect_results: Vec<_> = futures::future::join_all(disconnect_tasks).await
        .into_iter()
        .map(|r| r.unwrap())
        .collect();
...
async fn test_concurrent_rate_limiting_consistency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 并发发送消息测试限流
    let message_tasks: Vec<_> = (0..20).map(|i| {
    ...
    // 等待所有消息检查完成
    let message_results: Vec<_> = futures::future::join_all(message_tasks).await
        .into_iter()
        .map(|r| r.unwrap())
        .collect();

    // 统计成功和失败的消息数量
    let successful_messages = message_results.iter().filter(|result| result.is_ok()).count();
    let rejected_messages = message_results.iter().filter(|result| result.is_err()).count();

    println!("成功消息: {}, 被拒绝消息: {}", successful_messages, rejected_messages);
    ...
    println!("最终状态: 消息数={}, 连接数={}", final_status.0, final_status.1);
...
async fn test_mixed_concurrent_operations_consistency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..5 {
        ...
        tasks.push(tokio::spawn(async move {
            presence_manager.user_connected(room_id, user_id).await.unwrap();
    ...
    for i in 0..5 {
        ...
        tasks.push(tokio::spawn(async move {
            let _ = rate_limiter.check_message_rate(user_id).await;
            sleep(Duration::from_millis(i as u64 * 20)).await;
            presence_manager.user_disconnected(room_id, user_id).await.unwrap();
        }));
    }
    ...
    for &user_id in user_ids {
        let user_status = services.presence_manager.is_user_online(room_id, user_id).await?;
...
async fn test_race_condition_consistency() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 等待所有竞态操作完成
    let _race_results: Vec<_> = futures::future::join_all(race_tasks).await
        .into_iter()
        .map(|r| r.unwrap())
        .collect();
...
/// 清理测试环境
async fn cleanup_test_environment(redis_client: &Arc<Client>) -> Result<(), Box<dyn std::error::Error>> {
