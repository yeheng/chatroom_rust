...
impl From<ApplicationError> for ApiError {
    fn from(error: ApplicationError) -> Self {
        ...
        match error {
            AppErr::Domain(DomainError::InvalidArgument { field, reason }) => {
                ApiError::new(StatusCode::BAD_REQUEST, "INVALID_ARGUMENT", format!("{}: {}", field, reason))
            }
            ...
            AppErr::Domain(DomainError::MessageNotFound) => {
                ApiError::new(StatusCode::NOT_FOUND, "MESSAGE_NOT_FOUND", "message not found")
            }
            AppErr::Domain(DomainError::UserAlreadyInRoom) => {
                ApiError::new(StatusCode::CONFLICT, "MEMBERSHIP_EXISTS", "user already joined room")
            }
            AppErr::Domain(DomainError::UserNotInRoom) => {
                ApiError::new(StatusCode::FORBIDDEN, "NOT_ROOM_MEMBER", "user not in room")
            }
            AppErr::Domain(DomainError::RoomIsPrivate) => {
                ApiError::new(StatusCode::FORBIDDEN, "ROOM_PRIVATE", "room requires password")
            }
            AppErr::Domain(DomainError::RoomClosed) => {
                ApiError::new(StatusCode::FORBIDDEN, "ROOM_CLOSED", "room is closed")
            }
            AppErr::Domain(DomainError::OperationNotAllowed) => {
                ApiError::new(StatusCode::FORBIDDEN, "OPERATION_NOT_ALLOWED", "operation not allowed")
            }
            AppErr::Repository(_) | AppErr::Password(_) | AppErr::Broadcast(_) => {
            ...
            AppErr::Authorization => {
                ApiError::new(StatusCode::FORBIDDEN, "AUTHORIZATION_FAILED", "authorization failed")
            }
        }
    }
}
